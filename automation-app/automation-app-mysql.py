...
class Functions:
    ...
    def upload_file(self):
        self.file = filedialog.askopenfilename(
            title="Selecione o arquivo", filetypes=[("Arquivo excel", "*.xlsx")]
        )
        if self.file:
    ...
    def runcode_in_thread(self, runcode):
        self.status = messagebox.askyesno(
            title="Atenção", message="Você realmente deseja executar a automação?"
        )
        if self.status == True:
    ...
    def stop_run(self, automation):
        self.status = messagebox.askyesno(
            title="Atenção", message="Você realmente deseja parar a automação?"
        )
        if self.status == True:
            ...
            self.stoprunning_label.configure(
                text="Automação interrompida conforme solicitado.", text_color="#D61736"
            )
        ...
        for index, row in self.df.iterrows():
            try:
                ...
                if not self.keep_running:
                    break
                self.driver.find_element(
                    By.XPATH, '//input[@name="business_name"]'
                ...
                first_number = self.driver.find_element(
                    By.XPATH, '//span[@id="numb1"]'
                ).text
                second_number = self.driver.find_element(
                    By.XPATH, '//span[@id="numb2"]'
                ).text
                result = int(first_number) + int(second_number)
                self.driver.find_element(By.XPATH, '//input[@id="number"]').send_keys(
                    result
                )
                time.sleep(1)
                ...
                self.keep_running = False
                self.stoprunning_label.configure(
                    text="Falha na execução da automação.", text_color="#D61736"
                )
                break
        self.driver.quit()
...
class User_Interface(Functions):
    ...
    def login_window(self):
        ...
        title_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Login", text_color="#002621"
        )
        title_label.pack(padx=10, pady=10)
        # Username Title
        usernametitle_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Usuário *", text_color="#002621"
        )
        usernametitle_label.pack(padx=10)
        # Username Entry
        username_entry = customtkinter.CTkEntry(
            window, border_width=1.2, border_color="#002621", fg_color="#FFFFFF"
        )
        username_entry.pack(padx=10, pady=10)
        # Password Title
        passwordtitle_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Senha *", text_color="#002621"
        )
        ...
        forgot_password_button = customtkinter.CTkButton(
            window,
            border_width=0,
            command=self.redefinition_window,
    ...
    def register_window(self):
        ...
        register_observation_title = customtkinter.CTkLabel(
            ...
            text="Coloque o nome de usuário de seu departamento e depois registre uma senha.",
            text_color="#002621",
        )
        register_observation_title.pack(padx=10, pady=10)
        # Username Title
        usernameregistertitle_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Usuário", text_color="#002621"
        )
        usernameregistertitle_label.pack(padx=10)
        # Username Entry
        usernameregister_entry = customtkinter.CTkEntry(
            window, border_width=1.2, border_color="#002621", fg_color="#FFFFFF"
        )
        usernameregister_entry.pack(padx=10, pady=10)
        # Password Title
        passwordtitleregister_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Senha", text_color="#002621"
        )
        passwordtitleregister_label.pack(padx=10)
        # Password Entry
        passwordregister_entry = customtkinter.CTkEntry(
            window,
            border_width=1.2,
            border_color="#002621",
            fg_color="#FFFFFF",
        ...
        # Password Title
        passwordtitleconfirmation_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Confirme a senha", text_color="#002621"
        )
        passwordtitleconfirmation_label.pack(padx=10)
        # Password Confirmation Entry
        passwordconfirmation_entry = customtkinter.CTkEntry(
            window,
            border_width=1.2,
            border_color="#002621",
            fg_color="#FFFFFF",
        ...
        save_button = customtkinter.CTkButton(
            ...
            command=lambda: self.register_user(
                usernameregister_entry,
                passwordregister_entry,
                passwordconfirmation_entry,
            ),
            text="Salvar",
            text_color="#002621",
            border_color="#002621",
        ...
        self.register_message_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text=""
        )
    ...
    def redefinition_window(self):
        ...
        # Username Title
        redefinition_username_title = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Usuário", text_color="#002621"
        )
        redefinition_username_title.pack(padx=10, pady=10)
        # Username Entry
        redefinition_username_entry = customtkinter.CTkEntry(
            window, border_width=1.2, border_color="#002621", fg_color="#FFFFFF"
        )
        redefinition_username_entry.pack(padx=10, pady=10)
        # Password Title
        redefinition_password_title = customtkinter.CTkLabel(
            window, wraplength=win_width, text="Nova senha", text_color="#002621"
        )
        redefinition_password_title.pack(padx=10)
        # Password Entry
        redefinition_password_entry = customtkinter.CTkEntry(
            window,
            border_width=1.2,
            border_color="#002621",
            fg_color="#FFFFFF",
        ...
        redefinition_confirmation_title = customtkinter.CTkLabel(
            window,
            wraplength=win_width,
        ...
        # Password Confirmation Entry
        redefinition_confirmation_entry = customtkinter.CTkEntry(
            window,
            border_width=1.2,
            border_color="#002621",
            fg_color="#FFFFFF",
        ...
        save_button = customtkinter.CTkButton(
            window,
            border_width=1.2,
            command=lambda: self.password_redefinition(
                redefinition_username_entry,
                redefinition_password_entry,
                redefinition_confirmation_entry,
            ),
            text="Salvar",
            text_color="#002621",
            border_color="#002621",
        ...
        self.redefinition_message_label = customtkinter.CTkLabel(
            window, wraplength=win_width, text=""
        )
    ...
    def automations_test_window(self):
        ...
        title_label = customtkinter.CTkLabel(
            window,
            wraplength=win_width,
            text="Escolha abaixo a automação que deseja acessar",
            text_color="#002621",
        )
        title_label.pack(padx=10, pady=10)
        # Form Filling Button
        form_filling_button = customtkinter.CTkButton(
            window,
            border_width=2,
    ...
    def form_filling_window(self):
        ...
        title_Label.pack(padx=10, pady=10)
        # Insert File
        insertfile_button = customtkinter.CTkButton(
            window,
            border_width=2,
        ...
        # Finished Upload
        self.finishupload_label = customtkinter.CTkLabel(
            window, text="", text_color="#002621"
        )
        ...
        # Running label
        self.running_label = customtkinter.CTkLabel(
            window, text="", text_color="#002621"
        )
        ...
        stop_button = customtkinter.CTkButton(
            window,
            text="Parar",
            command=lambda: self.stop_run("form_filling"),
            fg_color="#D61736",
            hover_color="#AE122C",
        )
        ...
        self.progress_bar = customtkinter.CTkProgressBar(
            window, width=400, progress_color="#85DDA4"
        )
        ...
        back_to_automations_button = customtkinter.CTkButton(
            window,
            border_width=0,
